...
template <class Type, class Allocator = std::allocator<Type>> class small_ptr;

namespace detail {

    template <class Type, class Allocator> class small_ptr_policies
    {
        typedef pointer_policy::use_allocator<Type, Allocator> use_allocator;
    ...
    public:
        typedef pointer_policy::strict_weak_ordered<
            pointer_policy::pointer_access<
                pointer_policy::reference_count_shared<storage_policy>>>
            type;
    };
...
public:
    template <class... Arguments> explicit small_ptr(Arguments &&... arguments)
