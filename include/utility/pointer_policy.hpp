...
namespace utility { namespace pointer_policy {
    ...
    */
    template <class Type> class allocate_and_construct
    {
        ...
        */
        ...
        template <class Allocator> class guard<Allocator, true>
        {
        public:
    ...
    public:
        template <class Allocator, class... Arguments> Type * operator()(
            Allocator & allocator, Arguments &&... arguments) const
        {
            ...
            // goes wrong.
            static bool const is_safe =
                noexcept(Type(std::forward<Arguments>(arguments)...));
            ...
            Allocator const & allocator_, Arguments &&... arguments) const
        {
            ...
            return (*this)(allocator, std::forward<Arguments>(arguments)...);
        }
    };
    ...
    */
    ...
    public:
        use_allocator(Allocator const & allocator) noexcept(
            noexcept(Allocator(allocator)))
        ...
        */
        use_allocator(Type * object, Allocator const & allocator) noexcept(
            noexcept(Allocator(allocator)))
        : pointer_and_allocator_(object, allocator)
        {}

        template <class... Arguments> use_allocator(
            construct_as<Type>, Allocator const & allocator,
            Arguments &&... arguments)
        : pointer_and_allocator_(
              allocate_and_construct<Type>()(
    ...
    private:
        ...
        Type *& pointer_() { return pointer_and_allocator_.first(); }
        Type * pointer_() const { return pointer_and_allocator_.first(); }

        Allocator & allocator_() { return pointer_and_allocator_.second(); }
    ...
    */
    ...
    type.
    */
    template <class Storage> class with_recursive_type : public Storage
    {
    public:
        template <class... Arguments>
        with_recursive_type(Arguments &&... arguments) noexcept(
            noexcept(Storage(std::forward<Arguments>(arguments)...)))
    ...
    */
    template <class Storage> class reference_count_shared : public Storage
    {
    public:
        template <class... Arguments>
        reference_count_shared(Arguments &&... arguments) noexcept(
            noexcept(Storage(std::forward<Arguments>(arguments)...)))
        : Storage(std::forward<Arguments>(arguments)...)
        {
            static_assert(
        ...
        */
        reference_count_shared(reference_count_shared && that) noexcept(
            noexcept(Storage(std::declval<Storage &&>())))
        ...
        reference_count_shared & operator=(reference_count_shared const & that)
        {
    ...
    private:
        ...
        {
            if (!Storage::empty() && shared::release_count(Storage::object()))
                Storage::destruct();
        }
    ...
    */
    template <class Lifetime> class pointer_access : public Lifetime
    {
    public:
        template <class... Arguments>
        pointer_access(Arguments &&... arguments) noexcept(
            noexcept(Lifetime(std::declval<Arguments &&>()...)))
    ...
    */
    template <class Access> class strict_weak_ordered : public Access
    {
    public:
        template <class... Arguments>
        explicit strict_weak_ordered(Arguments &&... arguments) noexcept(
            noexcept(Access(std::declval<Arguments &&>()...)))
    ...
    */
    template <class Policies, class Derived> class pointer : public Policies
    {
    public:
        typedef typename Policies::value_type value_type;

        template <class... Arguments>
        explicit pointer(Arguments &&... arguments) noexcept(
            noexcept(Policies(std::declval<Arguments &&>()...)))
        ...
        */
        ...
        {
            return Derived(pointer(
                construct_as<Type>(), std::forward<Arguments>(arguments)...));
        }
    ...
    template <class Policies, class Derived> inline void swap(
        pointer<Policies, Derived> & one,
