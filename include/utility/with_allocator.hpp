...
public:
    with_allocator(with_allocator const & other) noexcept(
        noexcept(allocator_type(std::declval<allocator_type const &>())))
    : allocator_type(other)
    {}

    with_allocator(with_allocator && other) noexcept(
        noexcept(allocator_type(std::declval<allocator_type &&>())))
    : allocator_type(std::move(other))
    {}

    with_allocator(allocator_type const & allocator) noexcept(
        noexcept(allocator_type(std::declval<allocator_type const &>())))
    : allocator_type(allocator)
    {}

    with_allocator(allocator_type && allocator) noexcept(
        noexcept(allocator_type(std::declval<allocator_type &&>())))
