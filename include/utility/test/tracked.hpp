...
struct tracked_counts
{
    ...
    tracked_counts(
        ...
        int copy_assign_count, int move_assign_count, int swap_count,
        int destruct_count, int destruct_moved_count)
    : value_construct_count(value_construct_count), copy_count(copy_count),
      move_count(move_count), copy_assign_count(copy_assign_count),
      move_assign_count(move_assign_count), swap_count(swap_count),
      destruct_count(destruct_count), destruct_moved_count(destruct_moved_count)
    {}
...
class tracked_registry
{
    ...
    friend std::ostream & operator<<(
        std::ostream & os, const std::pair<tracked<> *, slot> & object)
    {
        return os << "tracked<" << object.second.info.name() << "> object at "
                  << object.first << " (allocation " << object.second.index
                  << ")";
    }
    ...
    static void exists_error(const std::pair<tracked<> *, slot> & object)
    {
        BOOST_ERROR(
            "tracked<" << object.second.info.name() << "> object at "
                       << object.first << " (allocation " << object.second.index
                       << ") not destructed.");
    }
...
public:
    tracked_registry() : index(), counts_() {}
    ...
    void insert(tracked<> * object, const std::type_info & type)
    {
        ...
        if (i != registry.end()) {
            BOOST_ERROR(
                "tracked<" << i->second.info.name() << "> object at " << object
                           << " (allocation " << i->second.index
                           << ") already exists where a tracked<" << type.name()
                           << "> object is now constructed (allocation "
                           << index << ")");
        } else
            registry.insert(i, std::make_pair(object, slot(type, index)));
        ++index;
    }
    ...
    */
    ...
    bool finished() const { return counts_.alive_count() == 0; }
...
public:
    ...
    tracked(
        tracked_registry & registry, std::type_info const & type = typeid(void))
...
template <class Content> class tracked : public tracked<void>
{
...
public:
    ...
    template <class OtherContent> tracked(
        tracked<OtherContent> const & other,
        typename boost::enable_if<
            std::is_convertible<OtherContent const &, Content>>::type * = 0)
    : tracked<void>(other), content_(other.content_)
    {}

    template <class OtherContent> tracked(
        tracked<OtherContent> && other,
        typename boost::enable_if<
            std::is_convertible<OtherContent, Content>>::type * = 0)
    : tracked<void>(std::move(other)), content_(std::move(other.content_))
    {}
...
template <class Content>
inline std::ostream & operator<<(std::ostream & os, tracked<Content> const & t)
{
    return os << t.content();
}
