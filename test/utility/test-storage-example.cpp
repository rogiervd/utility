...
public:
    ...
    typename storage::get<Type, simple_container &>::type content()
    {
        return content_;
    }
    typename storage::get<Type, simple_container const &>::type content() const
    {
        return content_;
    }
    ...
    typename Result::type content_value()
    {
        return content_;
    }
    template <class Result = typename storage::get_value<Type>>
    typename Result::type content_value() const
    {
        return content_;
    }

    typename storage::get_pointer<Type, simple_container &>::type pointer()
    {
        return &content();
    }
...
BOOST_AUTO_TEST_CASE(test_utility_storage_example_extensive)
{
    ...
    using utility::value_construct_count;
    {
        ...
        BOOST_CHECK_EQUAL(
            ...
            value_construct_count(1) + move_count(1) + destruct_moved_count(1));
    ...
    // Const container.
    {
        ...
        static_assert(
            std::is_same<
                decltype(c.content()), utility::tracked<int> const &>::value,
            "");
    }
    // Const contents.
    {
...
BOOST_AUTO_TEST_CASE(test_utility_storage_example_functions)
{
    ...
    example_struct s;
    {
        ...
        BOOST_CHECK_EQUAL(CALL_MEMBER_FUNCTION(s, member_function)("", 1.), 0);
        ...
        BOOST_CHECK_EQUAL(CALL_MEMBER_FUNCTION(s, member_function)("", 1.), 1);
    }
    {
        simple_container<int (example_struct::*)(std::string, double) const> c(
    ...
    {
        simple_container<int (example_struct::*)(std::string, double) volatile>
            c(&example_struct::example_function_5);
    ...
    {
        simple_container<int (example_struct::*)(std::string, double)
                             const volatile>
            c(&example_struct::example_function_7);
