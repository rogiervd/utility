...
template <class Storage> struct storage_tester : Storage
{
...
struct derived : base
{
    derived(utility::tracked_registry & registry, int i) : value(registry, i) {}
...
BOOST_AUTO_TEST_CASE(test_utility_use_new_delete_derived)
{
    ...
    storage_tester<storage_type> s1(
        utility::pointer_policy::construct_as<derived>(), registry, 7);
...
BOOST_AUTO_TEST_CASE(test_utility_use_allocator)
{
    ...
    typedef utility::test_allocator<std::allocator<value_type>> allocator_type;
...
void test_use_new_delete_allocation(utility::thrower & thrower)
{
    ...
    typedef utility::pointer_policy::use_new_delete<value_type> storage_type;

    // This may throw but should then deallocate immediately.
    storage_tester<storage_type> s1(
        utility::pointer_policy::construct_as<value_type>(), thrower, registry,
        7);
...
void test_use_allocator_allocation(utility::thrower & thrower)
{
    ...
    // This may throw but should then deallocate immediately.
    storage_tester<storage_type> s1(
        utility::pointer_policy::construct_as<value_type>(), allocator, thrower,
        registry, 7);
