...
template <class Type> inline auto return_const_ref(Type const & c) RETURNS(c);

template <class Type> inline auto return_ref(Type & c) RETURNS(c);
...
{
    {
        int i = 7;
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(return_const_ref(i)), int const &>) );
    ...
    BOOST_MPL_ASSERT(
        (std::is_same<decltype(construct_default<float>()), float>) );
