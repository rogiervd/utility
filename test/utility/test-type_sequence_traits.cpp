...
*/
template <
    template <class, class> class SinglePredicate,
    template <class, class> class AllPredicate>
struct check_binary_predicate
{
    template <class TypeA, class TypeB> void test_binary_with_one() const
    {
        static_assert(
    ...
    void test_binary_with_two() const
    {
        static_assert(
            AllPredicate<
                meta::vector<TypeA, TypeB>, meta::vector<TypeC, TypeD>>::value
    ...
    template <class Type1, class Type2, class Type3> void check() const
    {
        ...
        static_assert(
            !AllPredicate<meta::vector<>, meta::vector<Type1>>::value, "");
        static_assert(
            !AllPredicate<meta::vector<>, meta::vector<Type1, Type2>>::value,
            "");

        static_assert(
            !AllPredicate<meta::vector<Type1>, meta::vector<>>::value, "");
        static_assert(
            !AllPredicate<
        ...
        static_assert(
            !AllPredicate<meta::vector<Type1, Type2>, meta::vector<>>::value,
            "");
        static_assert(
            !AllPredicate<
...
{
    explicit explicitly_convertible_to(convertible_from const &) noexcept(
        false);
};

struct explicitly_nothrow_convertible_to
{
    explicit explicitly_nothrow_convertible_to(
        convertible_from const &) noexcept;
};

BOOST_AUTO_TEST_CASE(test_utility_are_convertible)
{
    static_assert(
        std::is_convertible<convertible_from, convertible_to>::value, "");
    static_assert(
        !std::is_convertible<
...
BOOST_AUTO_TEST_CASE(test_utility_are_constructible)
{
    ...
    static_assert(
        is_constructible<explicitly_convertible_to, convertible_from>::value,
        "");
    static_assert(
        !is_constructible<convertible_from, explicitly_convertible_to>::value,
        "");
...
BOOST_AUTO_TEST_CASE(test_utility_are_nothrow_constructible)
{
    ...
    check_binary_predicate<
        is_nothrow_constructible, utility::are_nothrow_constructible>
        test;
    test.check<
        convertible_from, explicitly_convertible_to,
...
BOOST_AUTO_TEST_CASE(test_utility_are_assignable)
{
    ...
    static_assert(
        ...
        "");
    static_assert(
        utility::is_assignable<assignable_to &, assignable_from const &>::value,
        "");
    static_assert(
        utility::is_assignable<
...
BOOST_AUTO_TEST_CASE(test_utility_are_nothrow_assignable)
{
    ...
    check_binary_predicate<
        utility::is_nothrow_assignable, utility::are_nothrow_assignable>
        test;
    test.check<assignable_from, assignable_to, nothrow_assignable_to>();
}
