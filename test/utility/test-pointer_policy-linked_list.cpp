...
template <bool DestructWithLoop> struct node : utility::shared
{
    ...
    node(int value) : value_(value), next_(std::allocator<node>()) {}
};
...
namespace utility { namespace pointer_policy {
    ...
    {
        utility::small_ptr<node<true>> && operator()(node<true> * object) const
        {
...
template <bool DestructWithLoop>
utility::small_ptr<node<DestructWithLoop>> make_list(std::size_t number)
{
